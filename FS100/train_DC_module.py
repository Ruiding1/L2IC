...
from model import G_NET_FS100, D_NET_FS100
from datasets_fs100 import *
import random
...
def load_network():
    # prepare G net
    netG = G_NET_FS100(cfg.FINE_GRAINED_CATEGORIES, fore_num=3, mask_num=1, back_num=1).to(device)
    netG.apply(weights_init)
    netG = torch.nn.DataParallel(netG, device_ids=gpus)

    netD = D_NET_FS100(cfg.FINE_GRAINED_CATEGORIES)
...
class Trainer(object):
    def __init__(self, output_dir):
        ...
        if not cfg.TRAIN.FLAG:
            G = "../saved_models/fs100/netG.pth"
            self.eval(path_G=G, path_D=None)
            print('===> Evaluation Completed ')
            ### ===> Evaluation Completed

    def prepare_data(self, data):
        aug_img, real_img, real_c, _ = data
...
if __name__ == "__main__":
    ...
    # (1.6538513135858484, 0.01403001465871591)
    # FID 14.384