...
# ==================== FS100 Generator ====================
class SubNet_FS100_C(nn.Module):
    def __init__(self, ngf, num_residual=1):
        super().__init__()
        self.ngf = ngf
        self.code_len = 0
...
class SubNet_FS100_B(nn.Module):
    def __init__(self, ngf, num_residual=1):
        super().__init__()
        self.ngf = ngf
        self.code_len = cfg.SUPER_CATEGORIES
...
class G_NET_FS100(nn.Module):
    def __init__(self, class_code, fore_num, mask_num, back_num):
        ...
        self.jointConv = sameBlock(self.code_len + ngf, ngf)
        # mask networks
        self.second_stage = SubNet_FS100_C(ngf, num_residual=mask_num)
        self.second_mask = GET_MASK(ngf // 4)
        # fore networks
        self.colour_stage = SubNet_FS100_C(ngf, num_residual=fore_num)
        self.colour_image = GET_IMAGE(ngf // 4)
        # back networks
        self.colour_stag_inverse = SubNet_FS100_B(ngf, num_residual=back_num)
        self.colour_image_inverse = GET_IMAGE(ngf // 4)

    def forward(self, z_code, c_code, c_code_inv):
        p_temp = self.parent_stage(z_code)
        # back network
        m_temp = self.colour_stag_inverse(p_temp, c_code_inv)
        bg_img = self.colour_image_inverse(m_temp)
        # class code
        h, w = p_temp.size(2), p_temp.size(3)
        cz_code = c_code.view(-1, self.code_len, 1, 1).repeat(1, 1, h, w)
...
class G_NET_FS100_syn(nn.Module):
    def __init__(self, class_code, fore_num, mask_num, back_num):
        super().__init__()
        ngf = cfg.GAN.GF_DIM
...
def encode_img_FS100(ndf=64, in_c=3):
    layers = nn.Sequential(
        nn.Conv2d(in_c, ndf, 4, 2, 1, bias=False),
        nn.LeakyReLU(0.2, inplace=True),
        nn.Conv2d(ndf, ndf * 2, 4, 2, 1, bias=False),
...
class D_NET_FS100(nn.Module):
    def __init__(self, c_dim, ndf=64):
        super(D_NET_FS100, self).__init__()
        self.ndf = ndf
        self.c_dim = c_dim
        self.base = encode_img_FS100()
        self.info_head = nn.Sequential(nn.Conv2d(ndf * 8, ndf * 8, 3, 1, 1, bias=False),
                                       nn.BatchNorm2d(ndf * 8),
                                       nn.LeakyReLU(0.2, inplace=True),
                                       nn.Conv2d(ndf * 8, ndf * 8, kernel_size=4, stride=4))
        ...
        self.mlp = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(ndf * 8, cfg.FINE_GRAINED_CATEGORIES),
            nn.Softmax(dim=1)
        )
        self.rf_head = nn.Sequential(nn.Conv2d(ndf * 8, 1, kernel_size=4, stride=4),
                                     nn.Sigmoid())
