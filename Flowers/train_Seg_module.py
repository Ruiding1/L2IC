...
def evaluate(netEncM, loader, device):
    ...
    for x_data in loader:
        ...
        iou = torch.max(
            ...
            (((pred_f + gt) >= 1).view(bs, -1).sum(dim=-1)+0.001),
            (pred_b * gt).view(bs, -1).sum(dim=-1) / \
            (((pred_b + gt) >= 1).view(bs, -1).sum(dim=-1)+0.001)
        )

        dice = torch.max(
            2 * (pred_f * gt).view(bs, -1).sum(dim=-1) / \
            (pred_f.view(bs, -1).sum(dim=-1) + gt.view(bs, -1).sum(dim=-1)+0.001),
            2 * (pred_b * gt).view(bs, -1).sum(dim=-1) / \
            (pred_b.view(bs, -1).sum(dim=-1) + gt.view(bs, -1).sum(dim=-1)+0.001)
        )
...
def load_network():
    # prepare G net
    netG = G_NET_DCS(cfg.FINE_GRAINED_CATEGORIES,fore_num=1,mask_num=2,back_num=1).to(device)
...
class Trainer(object):
    def __init__(self, output_dir):
        ...
        if cfg.TRAIN.FLAG:
            U = "../saved_models/flowers/netU.pth"
            netU = load_model(self.netU, U)
            trainset = FlowersDataset(cfg.DATA_DIR,
                                      "test",
            ...
            print("eval finished", "\t iou_s : ", "{:.3f}".format(iou_s), "\t dice_s : ", "{:.3f}".format(dice_s))
