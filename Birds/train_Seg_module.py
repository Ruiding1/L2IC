...
class Trainer(object):
    def __init__(self, output_dir):
        ...
        if cfg.TRAIN.FLAG:
            U = "../saved_models/birds/netU.pth"
            netU = load_model(self.netU, U)
            config = Config('./cub_split.yml')
            trainset = CubDataset(config, data_split=config.TEST_SPLIT, use_flip=False)
