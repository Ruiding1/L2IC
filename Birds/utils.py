...
def child_to_parent_structe(child_c_code, classes_child, classes_parent):
...
def child_to_parent(child_c_code, classes_child, classes_parent):
    ratio = classes_child / classes_parent
    arg_parent = torch.argmax(child_c_code, dim=1) / ratio
    parent_c_code = torch.zeros([child_c_code.size(0), classes_parent])
    for i in range(child_c_code.size(0)):
        parent_c_code[i][int(arg_parent[i])] = 1
    return parent_c_code
...
def save_img_results(imgs_tcpu, fake_imgs, count, epoch, image_dir, flage, nrow=8):
    ...
    if imgs_tcpu is not None:
        if flage == 0:
            ...
            vutils.save_image(
                real_img, '%s/count_%04d_same_c%d.png' % (image_dir, count, epoch),
            ...
            real_img = imgs_tcpu[:][0:num]
            vutils.save_image(
                real_img, '%s/count_%04d_same_p%d.png' % (image_dir, count, epoch),
                scale_each=True, normalize=True, nrow=nrow)
            real_img_set = vutils.make_grid(real_img).numpy()
            real_img_set = np.transpose(real_img_set, (1, 2, 0))
            real_img_set = real_img_set * 255
            ...
            real_img = imgs_tcpu[:][0:num]
            vutils.save_image(
                real_img, '%s/count_%09d_shuffle_images%d.png' % (image_dir, count, epoch),
                scale_each=True, normalize=True, nrow=nrow)
            real_img_set = vutils.make_grid(real_img).numpy()
            real_img_set = np.transpose(real_img_set, (1, 2, 0))
            real_img_set = real_img_set * 255
