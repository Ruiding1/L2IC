...
class SubNet_DCS_B_DIM(nn.Module):
    def __init__(self, ngf, b, num_residual=1):
        super().__init__()
...
class G_NET_DCS_DIM(nn.Module):
    def __init__(self, class_code, b_coode, fore_num, mask_num, back_num):
        super().__init__()
        ngf = cfg.GAN.GF_DIM
        ...
        self.colour_image = GET_IMAGE(ngf // 4)
        # back networks
        self.colour_stag_inverse = SubNet_DCS_B_DIM(ngf, b=b_coode, num_residual=back_num)
        self.colour_image_inverse = GET_IMAGE(ngf // 4)
...
class FEATURE_GENERATOR(nn.Module):
    ...
    def forward(self, z_input):
        out_code = self.fc(z_input).view(-1, self.ngf, 4, 4)
        out_code = self.upsample1(out_code)
        out_code = self.upsample2(out_code)
        out_code = self.upsample3(out_code)
        out_code = self.upsample4(out_code)
...
class G_NET_OF_ComGAN(nn.Module):
    def __init__(self, mask_num, fore_num, back_num):
        super().__init__()
        ngf = cfg.GAN.GF_DIM

        self.parent_stage = FEATURE_GENERATOR(ngf * 8)
        self.first_stage = MASK_(ngf // 4, num_residual=mask_num)
        self.first_mask = GET_MASK(ngf // 4)

        self.colour_stage = SUBNETS_(ngf // 4, num_residual=fore_num)
        self.colour_image = GET_IMAGE(ngf // 4)

        self.colour_stag_inverse = SUBNETS_(ngf // 4, num_residual=back_num)
        self.colour_image_inverse = GET_IMAGE(ngf // 4)
    ...
    def forward(self, z_code):
        ...
        mask_img = self.first_mask(f_temp)
        s_temp = self.colour_stage(p_temp)
        fore_img = self.colour_image(s_temp)
        s_temp_inverse = self.colour_stag_inverse(p_temp)
        back_img = self.colour_image_inverse(s_temp_inverse)

        return mask_img, (mask_img * fore_img + (1. - mask_img) * back_img)
